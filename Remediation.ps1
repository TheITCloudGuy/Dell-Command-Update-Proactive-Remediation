...
If ($Session -ne $null){
    ...
    Start-Process shutdown.exe -ArgumentList '/r /f /t 5400 /c "SERC IT Team - Updating Bios, Please ensure work is saved, Computer will reboot in 90 minutes"'
    ...
    if (Test-Path $CabPathIndexModel)
        ...
        if ($DCUBIOSVersion -gt $CurrentBIOSVersion)
            ...
            if ($Remediate -eq $true)
                ...
                CMTraceLog -Message  "   Running Command: Invoke-WebRequest -Uri $TargetLink -OutFile $TargetFilePathName -UseBasicParsing -Proxy $ProxyServer " -Type 1 -LogFile $LogFile
                ...
                if (Test-Path $TargetFilePathName)
                    ...
                    $BIOSPassword = "ENTER PASSWORD HERE"
                    $BiosLogFileName = $TargetFilePathName.replace(".exe",".log")
                    $BiosArguments = "/s /l=$BiosLogFileName /p=$BIOSPassword"
